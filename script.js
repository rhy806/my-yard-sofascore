...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (fileInput) {
    fileInput.addEventListener('change', (e) => {
      ...
      const reader = new FileReader();
      reader.onload = (event) => {
        selectedMediaData = event.target.result;

        // Показываем превью
        previewContainer.style.display = 'block';
        if (selectedMediaType === 'image') {
          previewContainer.innerHTML = `<img src="${selectedMediaData}" style="max-width: 100%; max-height: 200px; border-radius: 8px;">`;
        } else {
          previewContainer.innerHTML = `<video src="${selectedMediaData}" controls style="max-width: 100%; max-height: 200px; border-radius: 8px;"></video>`;
        }
      };
      reader.readAsDataURL(file);
    });
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      // Проверяем: должно быть заполнено ХОТЯ БЫ одно из полей (текст ИЛИ файл)
      if (!text && !selectedMediaData) {
        alert('Добавьте текст или прикрепите фото/видео!');
        return;
      }

      const posts = JSON.parse(localStorage.getItem('app_news') || '[]');
      posts.unshift({
        id: Date.now(),
        text: text || null,
        media: selectedMediaData || null,
        mediaType: selectedMediaType || null,
        date: new Date().toLocaleString('ru-RU', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })
      });

      localStorage.setItem('app_news', JSON.stringify(posts));
...
function loadNews() {
  ...
  newsList.innerHTML = posts.map(post => {
    // Формируем блок медиа
    let mediaHTML = '';
    if (post.media) {
      if (post.mediaType === 'image') {
        mediaHTML = `<img src="${post.media}" style="width: 100%; border-radius: 8px; margin-bottom: 10px; display: block;">`;
      } else if (post.mediaType === 'video') {
        mediaHTML = `<video src="${post.media}" controls style="width: 100%; border-radius: 8px; margin-bottom: 10px; display: block;"></video>`;
      }
    }

    // Формируем блок текста
    const textHTML = post.text ? `<p style="margin: 0 0 10px 0; color: #eee; white-space: pre-wrap;">${post.text}</p>` : '';

    return `
      <div style="background: #1e1e1e; padding: 15px; border-radius: 12px; margin-bottom: 12px; text-align: left;">
        ${mediaHTML}
        ${textHTML}
        <div style="color: #666; font-size: 11px;">${post.date}</div>
      </div>
    `;
  }).join('');
}
